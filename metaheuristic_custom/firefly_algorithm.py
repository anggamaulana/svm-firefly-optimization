...
class FireflyAlgorithm(BaseAlgorithm):
    ...
    def search(self, number_of_fireflies = 10, maximun_generation = 10, randomization_parameter_alpha = 0.2, absorption_coefficient_gamma = 1.0):
        ...
        for generation in range(maximun_generation):
            ...
            ##add logging
            solution_firefly_per_generation = self.__select_best_firefly_by_light_intensity(self.__fireflies)
            self.function_wrapper.logging({"generation":generation , "best_decision_variable_values": solution_firefly_per_generation.location_coordinates, "best_objective_function_value": solution_firefly_per_generation.light_intensity})
